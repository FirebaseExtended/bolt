...
util.methods(RulesSuite, {
  ...
  database: function(appName, appSecret) {
    ...
    // this.adminClient = this.ensureUser('admin').setDebug();
    this.adminClient = new rest.Client(appName, appSecret);
    this.anonClient = this.ensureUser('anon').setDebug();
    this.databaseReady();
  },
  ...
  ensureUser: function(username) {
    if (!(username in this.users)) {
      ...
      if (username == 'anon') {
        ...
        token = generateAuthToken(opt, this.appSecret);
      }
